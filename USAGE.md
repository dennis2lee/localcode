...
- **삭제**: `DELETE /api/sessions/{id}` — Web UI 오른쪽 패널의 **삭제** 버튼으로도 가능합니다. 세션과(저장 중이라면) 그 JSONL 로그 파일이 완전히 제거되며 되돌릴 수 없습니다. 그 세션에 진행 중인 턴이 있으면 거부됩니다(409). 백그라운드 태스크로 만들어진 자식 세션은 연쇄 삭제되지 않고 그대로 남습니다(`visible:false`라 목록에는 안 뜸).
...
1. **세션** — 현재 데몬에 있는 모든 세션(이름 또는 ID, 에이전트, 생성 시각) 목록. 각 항목에 **전환**/**이름변경**/**삭제** 버튼이 있고, **+ 새 세션**으로 새로 시작할 수 있습니다.
