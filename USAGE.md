...
세션 메타데이터(`~/.localcode/sessions/<id>.meta.json`)와 이벤트 로그(`<id>.jsonl`)는 둘 다 디스크에 저장되고, **데몬을 재시작해도** 세션 목록·대화 컨텍스트·`/cost` 누적치가 그대로 이어집니다. 시작 시 `session.LoadAllFromDisk`가 두 파일 쌍을 전부 읽어 세션 목록을 복원하고, `agent.Loop.RehydrateAll()`이 각 세션의 이벤트 로그를 replay해서 모델에게 보낼 대화 히스토리(툴 호출/결과, 압축된 요약 포함)와 토큰 사용량을 재구성합니다 — 화면에 이전 대화가 다시 보이는 것뿐 아니라, 모델 자신도 그 맥락을 그대로 기억한 채로 다음 메시지를 받습니다. 복원에 실패한 세션 하나(예: `.meta.json`이 깨진 경우)가 있어도 나머지 세션은 정상적으로 복원되고, 데몬 로그에 경고만 남습니다.
