...
- [1부. 시작하기](#1부-시작하기) — [실행 모드](#실행-모드) · [원격 데몬 + SSH 터널](#원격-데몬--ssh-터널)
- [2부. 설정](#2부-설정) — [config.json](#설정-파일-configjson) · [localcode mcp](#localcode-mcp로-mcp-서버-관리하기) · [권한 규칙](#세밀한-권한-규칙-permission) · [Hooks](#hooks-hooks) · [/login 인증](#login으로-인증하기)
- [3부. 프로젝트 컨텍스트](#3부-프로젝트-컨텍스트) — [Skills](#skills) · [AGENTS.md](#agentsmd-프로젝트-규칙) · [Auto Memory](#auto-memory-모델이-스스로-기록하는-메모리)
- [4부. 명령어와 화면 조작](#4부-명령어와-화면-조작) — [화면 조작 공통](#화면-조작-tui--web-ui-공통) · [/skill](#skill-명령) · [/init](#init-명령) · [사용자 정의 명령](#사용자-정의-명령-이름) · [/memory](#memory-명령) · [/config](#config-명령) · [/compact](#compact-명령) · [/usage](#usage-명령) · [그 외 로컬 명령](#그-외-로컬-명령-help-version-exit-q)
- [5부. 세션과 대화 관리](#5부-세션과-대화-관리) — [세션 전환](#세션-전환) · [이름 변경/삭제](#세션-이름-변경삭제) · [Context Window 관리](#context-window-관리-사용량-표시--자동-압축) · [세션 로그](#세션-로그) · [데몬 재시작과 세션 복원](#데몬-재시작과-세션-복원)
- [6부. Web UI](#6부-web-ui) — [오른쪽 패널](#web-ui-오른쪽-패널) · [드래그 앤 드롭 첨부](#파일-드래그-앤-드롭-첨부) · [상태 표시줄](#프롬프트-하단-상태-표시줄)
- [7부. 에이전트와 자동화](#7부-에이전트와-자동화) — [사용 가능한 툴](#사용-가능한-툴-모델이-호출) · [여러 에이전트 조합하기](#여러-에이전트-조합하기) · [Plan 모드](#plan-모드-같은-대화-안에서-에이전트-전환) · [백그라운드 태스크](#백그라운드-태스크) · [다른 모델로 전환하기](#다른-모델로-전환하기) · [로컬 LLM 붙이기](#로컬-llm-lm-studio-등-붙이기)
- [알려진 제약](#알려진-제약)
...
- **matcher**: `pre_tool_use`/`post_tool_use`에서만 의미가 있고, 툴 이름(`bash`, `write_file` 등)에 대한 정규식입니다. **툴 이름 전체와 일치해야 매치**됩니다(자동 앵커링) — `"bash"`는 정확히 `bash` 툴에만 걸리고, `"bash|edit"`(양자택일)나 `"mcp__github__.*"`(패턴)처럼 쓸 수 있습니다. 생략하면(빈 문자열) 모든 툴 호출에 실행됩니다.
...
- TUI는 입력창 위아래에 가로줄을 그려서 경계를 표시하고, 그 바로 아래에 현재 에이전트 상태 한 줄(`agent: ...`)이 항상 표시됩니다 — 다음 메시지부터 적용될 에이전트가 입력창 바로 옆에 보이도록 하기 위함입니다. Tab으로 전환해도 대화 내용(transcript)에는 아무 줄도 추가되지 않고, 이 상태 줄만 갱신됩니다.
- **모델이 응답하는 중에 메시지를 보내면 큐에 쌓입니다.** 대화창에 `[queued] <내용>`으로 바로 표시되고, 상태 줄에는 `(N queued)`가 붙습니다. 진행 중인 턴이 끝나는 즉시 큐의 첫 메시지가 자동으로 전송되며, 여러 개를 연달아 보내면 그만큼 쌓였다가 하나씩 순서대로 나갑니다. 단, `/`로 시작하는 명령이나 `exit`/`:q`는 큐에 쌓이지 않고 이전과 동일하게 현재 턴이 끝날 때까지 무시됩니다 — 나중에 그대로 전송하면 명령이 아니라 모델에게 보내는 일반 텍스트가 되어버리기 때문입니다.
- TUI 종료는 **Ctrl+C**
...
실행 중에 켜고 끌 수 있는 두 설정을 다룹니다 — **auto_compact**(context window 자동 압축, 아래 [Context Window 관리](#context-window-관리-사용량-표시--자동-압축) 참고)와 **show_tps**(프롬프트 하단 tokens/sec 표시). 데몬 전체에 적용되는 설정이라(세션 하나가 아니라), 어느 세션에서 바꿔도 그 순간부터 모든 세션에 적용됩니다.

- `/config` — 현재 설정값을 보여줍니다 (모델 호출 없음, 즉답)
...
현재 세션에서 지금까지 호출한 모델별 누적 토큰 사용량을 보여줍니다 (모델 호출 없음, 즉답). 달러 비용은 표시하지 않고 **토큰 수만** 보여줍니다 — 위 상태 표시줄의 context 사용률(가장 최근 호출 기준 스냅샷)과 달리, `/usage`는 세션 시작부터 지금까지의 모든 API 호출을 모델별로 합산한 값입니다 (같은 대화를 재전송할 때마다 매번 그 전체 분량이 과금되므로, "이 세션이 총 얼마나 썼는가"는 스냅샷이 아니라 합산이 맞는 계산입니다). 아직 아무 호출도 없었으면 사용량이 없다는 메시지만 표시됩니다.
...
- **TUI**: 시작할 때 기존 세션이 하나라도 있으면 터미널에 목록(세션 ID/agent/생성 시각)이 뜨고, 번호를 입력해 이어하거나 `n`(또는 빈 입력)으로 새 세션을 시작합니다. 같은 화면에서 `d<번호>`(예: `d1`)로 세션 하나를 지우고 목록을 다시 보거나, `da`로 전체 세션을 지울 수 있습니다(`da`는 "yes"를 직접 입력해야 진행되는 확인 절차가 있습니다).
- **Web UI**: 화면 오른쪽 패널에 세션 목록이 항상 떠 있습니다 (아래 [Web UI 오른쪽 패널](#web-ui-오른쪽-패널) 참고). 다른 세션으로 전환하면 화면이 지워지고, 그 세션의 이벤트 로그 전체(사용자 메시지·모델 응답·툴 실행 기록)가 처음부터 재생됩니다.

세션 목록은 `GET /api/sessions`로도 직접 조회할 수 있습니다 (백그라운드 태스크는 `visible:false`라 이 목록에는 안 뜨고, `GET /api/sessions/{id}/tasks`로 따로 봅니다).

#### 세션 이름 변경/삭제

세션은 ID로만 식별·재개되므로, 이름(`title`)은 순전히 화면 표시용입니다.

- **이름 변경**: `POST /api/sessions/{id}/rename` (`{"title": "..."}`) — Web UI 오른쪽 패널의 **이름변경** 버튼으로도 가능합니다. 성공하면 `session.renamed` 이벤트가 남습니다.
- **하나 삭제**: `DELETE /api/sessions/{id}` — Web UI 오른쪽 패널의 각 세션 항목에 있는 **delete** 버튼, 또는 TUI 시작 시 세션 선택 화면에서 `d<번호>`(예: `d1`)로도 가능합니다. 세션과(저장 중이라면) 그 JSONL/메타 파일이 완전히 제거되며 되돌릴 수 없습니다. 그 세션에 진행 중인 턴이 있으면 거부됩니다(409). 백그라운드 태스크로 만들어진 자식 세션은 연쇄 삭제되지 않고 그대로 남습니다(`visible:false`라 목록에는 안 뜸).
- **전체 삭제**: `DELETE /api/sessions` — Web UI 오른쪽 패널의 **delete all** 버튼, 또는 TUI 세션 선택 화면에서 `da`(확인 문구 입력 필요)로 가능합니다. 백그라운드 태스크 자식 세션을 포함해 데몬이 알고 있는 모든 세션을 한 번에 지웁니다. 어느 세션이든 진행 중인 턴이 있으면 **하나도 지우지 않고** 거부됩니다(409) — 부분 삭제로 애매한 상태가 남는 걸 방지합니다.

### Context Window 관리 (사용량 표시 / 자동 압축)

매 턴이 끝나면 provider가 보고하는 토큰 사용량(Bedrock/Anthropic/`stream_options.include_usage`를 요청한 OpenAI-compat 서버 모두 지원)을 세션에 `usage` 이벤트로 기록합니다 — 입력/출력 토큰 수, 모델의 알려진 최대 context 크기([internal/modelinfo](internal/modelinfo/modelinfo.go)의 best-effort 표, 모르는 모델은 128000 기본값), 채워진 비율(%), 초당 토큰 수(TPS)를 담고 있습니다. TUI/Web UI는 이 이벤트로 프롬프트 하단 상태 표시줄을 갱신합니다.

**자동 압축(auto-compaction)**: context 사용량이 **80%**를 넘으면(그리고 `auto_compact`가 켜져 있으면) 다음 메시지를 보내기 직전에 지금까지의 대화 전체를 모델에게 한 번 요약시키고, 그 요약 하나로 대화 히스토리를 교체한 뒤 새 메시지를 이어 보냅니다. 화면에는 `[system] 대화가 context 절약을 위해 압축되었습니다` 알림이 남습니다. 요약 실패 시(네트워크 오류 등) 압축을 건너뛰고 원래 히스토리 그대로 진행합니다 — 절대 대화를 막지 않습니다.

- 기본값은 켜짐. `config.json`의 `"auto_compact_enabled": false`로 기본값을 끌 수 있고, 실행 중에는 `/config auto_compact on|off`로 바로 켜고 끌 수 있습니다.
- TPS 표시도 `"show_tps": false` 또는 `/config show_tps on|off`로 껐다 켰다 할 수 있습니다.
...
세션 메타데이터(`~/.localcode/sessions/<id>.meta.json`)와 이벤트 로그(`<id>.jsonl`)는 둘 다 디스크에 저장되고, **데몬을 재시작해도** 세션 목록·대화 컨텍스트·`/usage` 누적치가 그대로 이어집니다. 시작 시 `session.LoadAllFromDisk`가 두 파일 쌍을 전부 읽어 세션 목록을 복원하고, `agent.Loop.RehydrateAll()`이 각 세션의 이벤트 로그를 replay해서 모델에게 보낼 대화 히스토리(툴 호출/결과, 압축된 요약 포함)와 토큰 사용량을 재구성합니다 — 화면에 이전 대화가 다시 보이는 것뿐 아니라, 모델 자신도 그 맥락을 그대로 기억한 채로 다음 메시지를 받습니다. 복원에 실패한 세션 하나(예: `.meta.json`이 깨진 경우)가 있어도 나머지 세션은 정상적으로 복원되고, 데몬 로그에 경고만 남습니다.

## 6부. Web UI

### Web UI 오른쪽 패널

화면 오른쪽 패널은 위에서부터 세 구역입니다:

1. **세션** — 현재 데몬에 있는 모든 세션(이름 또는 ID, 에이전트, 생성 시각) 목록. 각 항목에 **switch**/**rename**/**delete** 버튼이 있고, **+ new session**으로 새로 시작하거나 **delete all**로 전체 세션을 한 번에 지울 수 있습니다(둘 다 확인 대화상자가 뜹니다).
2. **Background tasks** — `Task` 도구나 백그라운드 태스크 API로 띄운 하위 작업의 실시간 상태.
3. **MCP 서버** — 현재 연결된 MCP 서버 이름 목록 (`GET /api/mcp-servers`). 설정은 됐지만 연결에 실패한 서버는 여기 안 뜨고 데몬 로그에 경고만 남습니다.

#### 파일 드래그 앤 드롭 첨부

입력창에 파일을 끌어다 놓으면 `POST /api/sessions/{id}/uploads`로 서버에 업로드되고(`~/.localcode/uploads/<세션ID>/<파일명>`), 그 절대 경로가 `[첨부 파일: ...]` 형태로 입력창에 자동 삽입됩니다. 실제 파일 내용을 모델에게 보여주는 건 아니고, 모델이 필요하면 자기 `read_file`/`bash` 툴로 그 경로를 직접 읽습니다 — 텍스트 파일에는 바로 유용하고, 이미지 등 바이너리는 모델이 텍스트로 읽을 수 없으니 참고용 경로만 남습니다.

#### 프롬프트 하단 상태 표시줄

입력창 바로 아래에 한 줄로 표시됩니다: 현재 에이전트/모델, context 사용률(70% 이상 노란색, 90% 이상 빨간색), TPS(켜져 있으면), 그리고 왼쪽의 작은 점 하나 — 모델과 통신 중이면 노란색으로 깜빡이고, 응답이 끝나면 잠깐 초록색으로 반짝인 뒤 꺼집니다.
...
- **TUI**: **Tab** 키로 config에 등록된 에이전트를 순서대로 순환 전환. 입력창 바로 아래 상태 줄에 `agent: <이름>  ·  model: <모델 ID>` 형태로 현재 에이전트와 그 프로필이 실제로 쓰는 모델 ID가 항상 표시됩니다 (전환해도 대화 내용에는 아무 줄도 남지 않습니다).
