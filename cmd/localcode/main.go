...
func run() error {
	...
	gui := flag.Bool("gui", gui.Available(), "open a native desktop window instead of the TUI (requires a build made with -tags gui; defaults to on for such a build, pass --gui=false to force the TUI instead)")
