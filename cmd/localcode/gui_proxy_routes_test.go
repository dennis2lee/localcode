...
func TestEveryRouteThePageCallsWorksThroughTheWindowProxy(t *testing.T) {
	...
	probes := []probe{
		...
		{method: "GET", path: "/api/sessions/groups", ok: only(200)},
		{method: "POST", path: "/api/sessions/groups", body: `{"names":["work"]}`, ok: only(200, 204)},
		{method: "POST", path: "/api/sessions/" + sid + "/group", body: `{"group":"work"}`, ok: only(200, 204),
			note: "the group above is made first, so this one lands rather than being refused for not existing"},
