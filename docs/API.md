...
| `GET /api/update/install-notice` | nothing | Always `200`. `{"notice": null}`, or `{"notice": {"version", "exit_code", "status", "meaning", "log"}}` once per recorded failure: the first answer carries it and marks it shown, so the reopened window draws one line about a failed install on its first load and stays silent after. Reads the local record only, so it answers with the network down. Cancelled and installed records answer null | none |
