...
| `POST /api/sessions/groups` | `{"names"}` the whole list, and `{"rename": {"from", "to"}}` when one of them is an old group under a new name | `200 {"names"}`. A name that is gone and not renamed is deleted; the sessions that were in it are left ungrouped | `400` bad body, a name that is empty, padded, over 100 characters, carries a control character or is listed twice, or a rename whose `from` is not a group or whose `to` is not in the list |
| `POST /api/sessions/{id}/group` | `{"group"}`, or `""` for ungrouped; must already be in the group list | `200` the session | `400` bad body, no such group, or the session is archived; `404` unknown session |
