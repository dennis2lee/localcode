...
| Reasoning display | Reads `reasoning_content` from DeepSeek, SGLang, LM Studio, llama.cpp, Ollama, and vLLM before 0.16. Reads `reasoning` from vLLM 0.16 and later, and OpenRouter. Live display only, with no log storage or later model input. A muse model's reasoning is a block labelled with its running time that folds to one line when the answer starts, in the Web UI and the TUI; `/fold-thinking off` or the settings Muse tab turns that off. |
