...
| Effort | `off`, `low`, `medium`, `high`, or `xhigh`, set by profile, by `/effort`, by `/effort-set`, or by the control beside the model in the line under the prompt box. Only the levels a model tells apart are offered, and one it does not is refused: an adaptive Claude has one switch, muse has four steps, and the `reasoning_effort` vocabulary stops at `high`. The answer is kept per model, so a conversation that changes model finds the level it gave for the one it is on. OpenAI-compatible providers receive `reasoning_effort`, capped at `high`; muse models read the level from a system prompt line instead, which is where their model card puts it; Anthropic receives extended thinking; Bedrock receives `additionalModelRequestFields`. An unset value sends no field. |
...
| Reasoning display | Reads `reasoning_content` from DeepSeek, SGLang, LM Studio, llama.cpp, Ollama, and vLLM before 0.16. Reads `reasoning` from vLLM 0.16 and later, and OpenRouter. Splits a leading `<think>…</think>` off the answer, which LM Studio sends with its separate-reasoning setting off. Never sent back to the model. A muse model's reasoning is a block labelled with its running time that folds to one line when the answer starts, in the Web UI and the TUI, and is kept in the session log so a reload draws it again; `/fold-thinking off` or the settings Muse tab turns that off. Other models' reasoning is live display only. |
...
| Archive and retrieve | Archived conversations retain their event logs and remain readable. The logs stay on disk: startup does not read them, so a large archive costs no start time, and the first request that needs the events reads them. Booked work is disarmed while archived and armed again on retrieval. Existing background tasks can still record results. New work receives HTTP 403, enforced by the store. |
| Undo a turn | `/rewind` removes the last exchange from model history and restores files changed by `write_file` or `edit`. Exclusions: shell writes, background agent writes, user edits, symbolic link paths, and files over 8 MiB. `/redo` puts back the turn `/rewind` just undid, files included, and is refused once the conversation has moved on. |
...
| Hooks | Tool hooks: `pre_tool_use`, `post_tool_use`, `user_prompt_submit`, `stop`, and `session_start`. Lifecycle hooks: `pre_model`, `post_model`, `delegate`, `compact`, and `retry`. `pre_model` can block a request or inject context. `delegate` can reject a child task. Each hook uses the workspace of the source session. Per-hook `"timeout"` and `"fail_closed": true`, which blocks the action when a hook cannot finish; off by default, and covering not-finishing only. |
| MCP | `stdio`, streamable `http`, and `sse` transports. Authentication headers are withheld after redirects to another host. `localcode mcp add/list/get/remove` updates config.json without printing header or environment values. `mcp import-claude` imports Claude Code settings. Tool surface changes produce one startup warning before the stored fingerprint is updated. `/mcps` lists every server and switches one off per conversation; `/reset-mcp` reconnects every server for every conversation. |
| Egress | `network.egress` bounds localcode's own outbound connections: the model providers, remote MCP servers, the update check. Host names and `*.base`, loopback always allowed, enforced only when asked for. Shell commands and subprocess MCP servers are separate processes with their own sockets and out of scope. |
...
| Desktop window | Experimental native Web UI without a separate browser or visible server. Optional `-tags gui` build. `LocalCode.app` on macOS and `localcode-gui.exe` in the Windows MSI. See [USAGE.md](USAGE.md#desktop-window-experimental). |
