...
| [4. Commands and screen controls](#part-4-commands-and-screen-controls) | [What stop reaches](#what-stop-reaches), [Screen controls](#screen-controls), [Running a skill](#running-a-skill), [/init](#init), [Custom commands](#custom-commands), [Shell escape](#shell-escape), [/tasks](#tasks), [/memory](#memory), [/config](#config), [/compact](#compact), [/clear](#clear), [/rewind](#rewind), [Sampling](#sampling-temperature-top_p-top_k), [/review](#review), [/redo](#redo), [/model-invocable](#model-invocable), [/usage totals](#usage-totals), [/export](#export), [/mcps](#mcps), [What the transcript shows](#what-the-transcript-shows), [Where localcode may connect](#where-localcode-may-connect), [What a session log holds](#what-a-session-log-holds-and-who-can-read-it), [/llm-doctor](#llm-doctor), [/context](#context), [The switches](#the-switches), [Other local commands](#other-local-commands) |
...
| [6. Web UI](#part-6-web-ui) | [Resizing and hiding the panels](#resizing-and-hiding-the-side-panels), [Left panel: sessions](#left-panel-sessions), [Right panel](#right-panel), [Drag and drop attach](#drag-and-drop-file-attach), [Pasting an image](#pasting-an-image), [Status bar](#status-bar-under-the-prompt), [Settings](#settings), [Switching agents with Tab](#switching-agents-with-tab), [Markdown rendering](#model-output-renders-as-markdown), [Watching a long turn](#watching-a-long-turn), [Redirecting a turn](#redirecting-a-turn-while-it-runs) |
...
| `mcp_servers` | Same shape as Claude Code's `.mcp.json`, so existing entries copy over directly. Opencode spelling `mcp` is also accepted. |
| `permission` | Fine grained allow/ask/deny rules per tool. Opencode spelling `tools` with boolean values is also accepted. See [Permission rules](#fine-grained-permission-rules). |
...
With the guard on, a turn whose steps only repeat tool calls it has already made is ended after a fixed number of them, with a notice naming the calls. This applies to every model. It is off by default.

| Control | Scope | Behavior |
|---|---|---|
| `/repeat-limit on`, or the ticked box | Daemon | Turns the guard on at 3 steps. |
| `/repeat-limit <steps>` or the number in the settings window | Daemon | Steps in a row that may add nothing new before the turn ends, at most 50. Saved to config.json as `repeat_limit`. |
| `/repeat-limit off` or the unticked box | Daemon | The default. A turn is never ended for repeating itself; a model that will not stop runs until you stop it. |
| `/repeat-limit` | Daemon | Reports the current ceiling. |

A repeat is a call with the same tool and the same arguments as one made earlier in the turn, so a model alternating between two earlier reads is repeating even though no two consecutive calls are identical. The guard exists because a model that asked for the same tool a thousand times once held a session busy for everything typed afterwards; that particular runaway, a reviewer calling `Verdict` after its verdict, is now caught by the tool ending the turn, and the guard is the second layer. Some local models think by re-reading the same two places, and for them the guard ends turns that were about to act. That is a judgment the person watching can make and the guard cannot, which is why it is a setting and why it starts off.
...
The `` !`cmd` `` splice is one of two readings of `!`. The other is the [shell escape](#shell-escape), which runs a command typed at the prompt.

### Shell escape

Typing `!` followed by a shell command runs it in the session's workspace and puts its output in the conversation, without reaching a model on the way in: `!git status`, `!cat ~/abc.txt`. The model sees the output on the next turn, with the transcript showing you ran it. This is a shell escape, the thing an editor has always had: a person typing in their own window runs what they could have run in their own terminal. So it does not consult the permission rules, including a `deny`. That is a deliberate hole in an otherwise closed surface. A model can never cause one: a delegated or model-composed prompt starting with `!` is answered as text, never executed.

Start with `!!` to send an ordinary message beginning with `!`. A lone `!` says what to run instead. Empty output shows `(no output)`. A failure shows its exit status, so a typo never reads as an empty answer. Output is handed back whole: neither the `bash` tool nor the turn loop trims what a command wrote, and trimming here would make `!` and the tool disagree about the same command. `!find /` puts everything it prints into the conversation. A command past the `bash` tool's timeout is reported as killed after it.
...
| `/reset-skills` | Reloads skills and custom commands from disk, against the live workspace. A skill or command installed or edited mid-run applies immediately, and the name completes like any other. |
...
Pasting an image into the prompt box attaches it to the message, and the model receives the picture rather than a path. That is the difference from dropping a file above: a drop uploads bytes and hands the model a filename to read, which works for text and cannot work for a screenshot. The desktop window is this same page, so pasting works there too.
