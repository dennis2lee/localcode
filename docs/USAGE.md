...
| Reply reached `max_tokens` | The notice under the reply names which limit cut it. The profile's own `max_tokens`: raise it, or set one if the profile has none and the built-in 4096 applied. The window: `/compact`, or a larger `context_window` if the figure in use was guessed or set too low. Both, when neither alone would let the next reply run longer. `/compact` is promised only when it helps even with a long summary, and offered with that condition when it helps only with a short one. |
