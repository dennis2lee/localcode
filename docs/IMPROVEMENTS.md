...
20. **Update rollback and installer verification. Mostly closed in v0.50.0; handoff since v0.86.0; remaining gaps open.**
    ...
    * The settings window's MSI path was run end to end on a GitHub Windows runner for v0.149.0: two versions built from the branch, the older installed, the button pressed, and the newer installed with the window reopened; a second request while one waited; and an install blocked by policy, which left the old version and reopened it. The workflow lived on a diagnostic branch and is not part of the release checks. `internal/update` tests use a controlled server. The startup handoff on Windows is verified in parts on the Windows CI runner (two-process handoff, rename install, window proxy); the assembled startup path needs a release server and has not been run end to end.
    * The helper copy, the installer log and the install record under `%LocalAppData%\localcode\updates` are written by localcode, and the MSI's uninstall does not remove them.
...
42. **Handoff for `/update` inside the desktop window. Done in v0.95.0.**
    ...
    * Superseded in v0.149.0: the registration is removed. The window closes itself before the installer runs, so the Restart Manager never closed it and had nothing to restart. A helper runs the MSI after the window exits and starts the window again.
