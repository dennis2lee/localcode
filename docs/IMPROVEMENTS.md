...
11. **Deny rules can be evaded by shell quoting.** Permission matching is a glob against the command text, and the text still contains its quotes — so a deny rule `curl *` is not matched by `"curl" …`, `cu''rl …`, or `c\url …`, all of which run `curl`. Normally that only weakens a deny to a prompt, but with `skip_permissions` on the prompt resolves to allow, so an explicitly denied command runs unprompted. v0.33.2 fixed the related bypass where an escaped quote hid a whole chained command (that one auto-allowed without `skip_permissions`), but this needs the subject unquoted and normalized before matching, which is a separate change.
...
14. **Compaction summaries sit in the event log as plain text.** v0.12.0 started storing the full summary in the `compacted` event so restarts can restore it. If a session contained sensitive material, the summary of it now lives in the log file too. Worth reviewing log file permissions and retention against that.
