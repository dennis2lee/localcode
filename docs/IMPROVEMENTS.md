...
| Workspace is process-wide | A turn in *any* session refuses a workspace change, because the working directory is one process-wide thing. v0.37.0 made that recoverable — the refusal names the sessions holding it up and offers to stop them — but the underlying limit stands: two clients on one daemon share a workspace. Resolving relative paths per session at the tool layer, instead of relying on `os.Chdir`, is what would actually fix it. |
