...
18. ~~**The desktop window still has a Windows caption.**~~ Done in v0.44.0, the way this entry said it would have to be: the window procedure is subclassed for `WM_NCCALCSIZE` and `WM_NCHITTEST`, and the page draws the buttons. What is still open is that **it has never been run on Windows** — this repository is developed from a Mac, and the CI job builds the binary without opening a window. `LOCALCODE_TITLEBAR=1` exists as the way back to the system frame precisely because of that. The geometry the two halves share is guarded by a test (`internal/gui/chrome_test.go`), but nothing can guard the hit test itself except trying it.
