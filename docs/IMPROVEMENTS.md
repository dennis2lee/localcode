...
6. **Switching to a long session shows only its tail.** A conversation opens at its end (the last ~400 events, cut back to a turn boundary) rather than replaying the whole log — measured at 1.63MB/751ms of client work for a 7,680-event session, against 0.08MB/4ms for the tail. What is still open is scrolling *back*: there is no "load earlier" control, so the beginning of a long conversation is only reachable from the session's `.jsonl`. The daemon already takes `?since=`, so the missing part is a client one.
...
8. **Compaction can fail when history already exceeds the context.** If the history is right at the model limit, the summarization request itself can fail. A truncation fallback that drops the oldest turns would make auto compaction more robust.
