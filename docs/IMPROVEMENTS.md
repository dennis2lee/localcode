...
18. **The desktop window still has a Windows caption.** v0.43.0 removed the title bar on macOS by drawing it transparent over the window's own background, which the OS supports directly. Windows has no equivalent: removing `WS_CAPTION` also removes the move, resize-from-top, minimise and close behaviour that lives in the non-client area, so it means subclassing the window procedure for `WM_NCCALCSIZE` and `WM_NCHITTEST` and drawing the buttons in the page. Wants a Windows machine to build it on, since a mistake there is a window that cannot be closed.
19. **Whisper's hallucination guard now rests entirely on localcode's own VAD.** v0.43.0 dropped `--no-fallback`, which was discarding any segment whose decode looked unconfident and taking fast speech with it. What stops a plausible sentence being invented out of room tone is now only that audio is not sent until the energy detector has heard speech, plus the annotation stripping in `cleanTranscript`. If invented sentences start appearing in quiet rooms, this is where to look, and the fix is a better detector (item 13) rather than the old flag.
