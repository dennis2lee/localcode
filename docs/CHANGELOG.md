...
* **`/usage` counts the prompt cache.** Under a working prompt cache the repeatedly sent history is not in `input_tokens`: the provider serves it from the cache and reports it as a cache read, and a newly cached prefix as a cache write, each billed at the provider's own cache rate. `/usage`, `/usage all`, the Web UI usage window and `localcode run --format json` counted input and output alone, so a cached session read as having sent a few thousand tokens when it had sent hundreds of thousands. Each model's line now shows `cache read` and `cache write` where a provider reported them, and `total` counts them. An OpenAI-compatible server's cached prompt (`prompt_tokens_details.cached_tokens`) is reported as a cache read instead of as fresh input. A conversation logged by v0.145.0 shows its cached prompt as one `cache read or write` figure, because that version recorded it without the split.
* **The Web UI usage window draws the `/usage all` figures, read from the daemon.** It added the totals up itself from the session list, which shows neither the sessions of sub-agents, scheduled runs and debate reviewers nor where a fork's copy of another log ends, so the window and the command gave two different totals for the same daemon. The daemon serves them as data (`GET /api/usage`), and the window draws the cache figures as their own bar segments, with a key naming the colours, the scale, and what the cache figures are.
* **Automatic compaction measures what the next request will carry.** It decided from the provider's last count alone, which cannot see anything appended after it: the output of a `!` command, a delegated turn's answer, the tool results of a turn whose follow-up request failed. A conversation the count put at 3% went to the provider at 65% with the threshold at 50%. It now adds an estimate of the text appended since the count, against the window of the profile the next message goes to. Images appended since the count are left out, so screenshots just pasted into a retried turn are not summarized away on an estimate of their size, and the model's reasoning is left out, so a live session and the same session restored from its log decide alike. With no count, after `/rewind`, the conversation's text is measured.
* **Automatic compaction does not run where it cannot help.** A conversation no longer than what a compaction would put in its place is left alone. A conversation that is a previous compaction's summary is not compacted again until the text that followed it is longer than it. Where the system prompt alone reaches the threshold, no compaction can bring the conversation under it, and it compacts once the conversation is half the room the system prompt leaves rather than on every turn.
* **`localcode run --format json` reports the whole run's usage.** Its `usage` object was the last model call's. It is now every call the run made, its sub-agents' and background sub-agents' included, with the cache figures and the number of calls (`GET /api/sessions/{id}/usage` through a daemon). A one-shot run waits for its background sub-agents before it reports rather than after.
* **`/usage all` and the usage window count sessions, not conversations.** Sub-agents', scheduled runs' and debate reviewers' sessions are counted, and none of them is a conversation in the list.

**Fixed**

* **`localcode run --session` through a daemon waits for its turn.** The daemon takes the message with 202 at once and runs the turn after it, and the run finished two seconds after the 202: a model slower than that gave an empty result, no usage, and exit 0. The run now ends at the turn's own end, after the background sub-agents it started have finished, and a turn that failed on the daemon fails the run.
* **`/usage all` counts a forked conversation's copied calls once.** A fork copies its source's log, usage included, and each copied call was counted again, dated the day of the fork. `session.forked` now records how many events it copied, and totals across conversations skip them.
* **Both clients blank the context percentage when a debate collapses its rounds, or a trim drops the oldest messages to fit the window.** The daemon drops its usage count at both, and the status line kept a fill for a history that was gone. `debate.ended` says whether it collapsed anything, and a trim's notice says it replaced the history.
* **A restart agrees with the live session about a debate.** A compaction inside a debate no longer stops a restart from collapsing its rounds, and a restart no longer keeps a usage count taken over rounds it collapsed.
* **Stop pressed while a debate's reviewers were reading ends the debate as stopped.** It was reported as "no review came back", with an error line for each reviewer. A reviewer that failed for its own reason before Stop still says so, and a debate whose author's turn failed says that rather than "no review came back".
* **An instruction typed while a turn was running keeps its tag after a restart.** A conversation rebuilt from its log held the instruction as untagged text inside tool output, where the live one marks it as the person's.
* **The window's title-bar buttons are guarded against a rename.** The page and the desktop window meet at one function name and a set of command strings, and a rename or an unknown command on either side would have left the Windows window with no working minimise, maximise or close. Tests now read every call and every binding on both sides.

## v0.145.0

A window that says where it came from, a cut-off reply that says which limit cut it and what would change the next request, and a count that includes what the cache served.

**Changed**

* **`/context` says where the context-window figure came from**: set by `context_window` in config.json, reported by the server, or guessed from the model name because the server did not report one. A guessed figure is the likeliest reason a window looks full when it is not, and nothing printed could say so. When the window shrinks the next reply below the profile's reservation, `/context` also says what the next reply actually gets and the figure that was sized against, and its conversation figure is measured by the same estimator, so the lines of one report add up.
* **A reply cut off at its length cap is followed by a notice that names which limit cut it and prescribes only a move that would send more next time.** The profile's own `max_tokens`: raise it. A profile that sets none is told the built-in default applied and to set one. The window: `/compact`, priced by what a compaction leaves behind (the system prompt, the summary's header and notes, and a summary of unknown length), so it is promised when it helps even with the longest summary a compaction keeps, offered with the summary length that would still help when only a shorter one would ("/compact makes room, if the compaction's summary comes out at 1800 tokens or fewer"), and not offered when no summary would. Both, when neither works alone. A window too small for any of it is told so with every term of the arithmetic, and a larger `context_window` is offered wherever the figure was not set in config.json. The notice judges the reply by the request that produced it, names the profile by its key, and never tells a person to raise a number that was not the one in the way.
* **A profile's `max_tokens` below 1024 is sent as written.** The floor that stops a request being shrunk to a few dozen tokens was returned as it was, so a profile asking for 512 sent 1024 whenever the window was nearly full.

**Fixed**

* **The provider's count is the whole prompt it read, cached prefix included.** Where a prompt cache is working the provider reports the prefix it served apart from what it counted fresh, and `input_tokens` covers only the suffix: this repo's own fixture has `input_tokens` 12 beside `cache_read_input_tokens` 4096. Read apart, a nearly full conversation looked almost empty. The next request was sized as though the prefix were not there, the context gauge in both clients read near zero, the sentence the model reads about remaining room overstated it by the whole prefix, and auto-compaction never fired. The cached prefix is recorded, written to the log, read back, and counted wherever the window is the question. It is kept out of `input_tokens`, which is what was billed at the full rate.
* **A request is sized against what the conversation holds now, not against the provider's last count alone.** The count describes the messages it was given; a tool result appended after it was invisible to it, and a tool result can be a quarter of the window. The estimate is now the count for what it covered, the reply included, plus a character estimate of only what was appended since. An image is counted at the ceiling the providers price a large image at, where a character count saw sixteen bytes of framing. Text a `pre_model` hook injects is counted before the request is clamped. A session restored from a log written before the measurement was recorded falls back to the larger of the two until its next turn.
* **Replacing a history drops the count that described it.** Six places did this beside the replacement and a seventh, collapsing a debate, did not, so the next request after a debate was sized against rounds that were no longer sent. It is a property of replacing a history now, and a session restored after a debate reaches the same state.
* **Both clients blank the context percentage when the history is replaced.** After `/clear`, a compaction, a rewind or a redo the daemon drops its count and says nothing until the next turn reports; the status line went on showing the old fill over a conversation that was nearly empty. The rate and the model name stay.
* **A compaction keeps at most 5120 tokens of the summary the model wrote.** The summarization request allows 4096, but nothing held what came back to it, so a server that ignores `max_tokens` had its whole answer stored and a compaction could leave the window fuller than the notice above priced it at. The bound is the cap and a quarter, for the difference between the model's tokenizer and the daemon's estimate. A longer summary is cut so that it and a note saying so fit in the bound.
* **A reply whose stream died half-way stays out of the history after a restart.** Live, a failed response is not a turn and is not sent back; the log closed it as a finished reply, so a restarted session rebuilt the half answer into its history and sent it on the next request. When the stream died after the model's tool call and before any text, nothing closed the reply at all, and a restarted session waited for that call's result and folded the next turn's answers into one message. The `message.part.end` of a failed reply now carries `failed: true`, with `text` empty when only the tool call had arrived, and rebuilding skips it and drops the calls it started. A log written before the key existed is rebuilt by the error that follows instead. A cancelled reply is kept, as it is live.
* **Pressing Esc during a reply no longer ends in an error line a third of the time.** A cancel closes the connection and the read fails with the cancel; the provider adapters offered that as a stream error or a clean close depending on which of two ready channels Go picked, so the same Esc kept the partial reply on some presses and dropped it with a red line on others. A read that fails after the turn was cancelled is the cancel, on all three adapters.
* **A restart rebuilds the compaction summary with the note about what it replaced.** The note naming the assets the replaced messages carried was stored beside the summary live and recorded on the event only by name, so the rebuilt history was shorter than the live one and the model after a restart was not told what its summary had replaced.
* **A conversation read through a `#S` reference does not offer a failed half reply as its last answer.** The summary skips it and the transcript marks it.
* **The Web UI closes out a tool row left running by a stream that died after the model asked for it.** No result was coming, and the row sat spinning under the error line for the life of the page.
...
* **localcode would not start on a machine that also had opencode installed.** A key localcode cannot honour is refused, which is right for a file somebody wrote for localcode and wrong for a file they wrote for another program — and opencode's own configs routinely carry `lsp`, `formatter` and `share`. A working `~/.localcode/config.json` two directories away made no difference. A refusal from an opencode file is now said out loud and that file is set aside; a refusal from localcode's own config, or from one named with `--config`, still stops everything. Two spellings of one opencode file in the same directory is the same kind of problem and is treated the same way.
* **Reading an opencode config could change how an existing one routes.** Every `opencode:` profile is held back from the Smart Agent lane scan, not only the per-agent ones, and the `default_profile` fallback is consulted among the profiles that may take a lane. Without both halves, a file that sets `model` handed its model back for every lane whose markers matched nothing — which is every lane when somebody's own profiles are local models the heuristic cannot classify — and flipped `Solo` with it, changing what the main model is told. With nothing hand-written to route to, the file's own model serves; a profile synthesised for one agent never does.
* **`"tools": {"write": false}` beside `"permission": {"write_file": …}` was accepted.** The collision check compared literal key text, while `write` becomes `edit`, which covers `write_file` — and resolution prefers the exact name over the alias, so the file denied edits in one spelling and allowed them in the other. The check now compares what the two keys cover.
* **An ordinary opencode repository could not be read.** An agent that names no model stopped the whole config loading, and that is most of the agents in an opencode file; it takes the default now, which is what `ResolveProfile` does for it anyway. A name that resolves to nothing is still refused, because a typo is a name. And a project `opencode.json` carrying only `"model"` was set aside for naming a provider "this config does not define" — the ordinary opencode layout, with credentials in the global file — when whether a provider exists is the merged configuration's question and `Validate` already asks it there.
* **A provider whose key lives in an environment variable.** opencode names the variable rather than the key, which is the one thing substitution cannot do on its own. Writing a placeholder and expanding a second time substituted values that came back from the first; normalising before substitution instead made every value the normaliser *reads* — a model string, an npm name, a base URL — unrecognisable when it was kept in a variable. Neither: the variable is reported and applied to that field after the document is decoded.
* Refusals collect and are reported together instead of one per start; they quote what the file contains, so `"snapshot": true` is accepted rather than refused with a sentence about `false`, and a `share` that is not a string is quoted as written; the agent refusal names `mode` when the file says `mode`; a `whitelist` no longer rejects the model it lists and a `blacklist` no longer lets through the one it forbids when a `models` block renames what it serves; the two MCP spellings are joined when their server names are disjoint and refused only on a shared name; and the same bytes give the same message, which they did not while a map was walked in map order.
* A file that was set aside is reported as a file rather than under "ignored keys", which had told people localcode read the rest of it.
...
The two worth naming. **v0.103.0 called four faults update-only and one of them was not**: the latched permission indicator is ordinary page code that any browser reaches by stopping a turn waiting on a permission request, and the test shipped alongside it drives no window and no proxy. The same paragraph then said everything was fine through v0.100.0 immediately after saying the window's update failed every time through v0.100.0; both cannot be true, and what was true is that the update failed too early to reach the faults behind it.

**v0.108.0 said no prefix cache could hit either system prompt.** They are identical up to the agent prompt at order 30 — base system, skills index, memory, workspace and project rules come first and are shared — so a cache holds that head. What a delegation costs is the divergent tail plus the sub-agent's whole context, which is enough to matter and is not what was written.

The rest are smaller and of one kind: a count stated as complete that was not (46 of 47 URLs, the multipart upload not among them), an event named `turn.end` that is `turn.done`, a number attached to the wrong noun, a control called the only filled one on the page when the jump-to-latest lozenge is another, a rule described at the call when it is enforced at the step, and three tests described as corrected when one was corrected and two were new.
...
The guard's own comment had said what the test should be — re-running the build after fixing what broke it is work, because the fix is a call of its own; re-running it to admire the result is not — and "a call this turn has not made before" was the wrong proxy for it. A carry-on is now earned only by a step that is both new **and** a change: a call this turn has not made, alongside a `write_file`, `edit` or `bash`. `read_file`, `grep`, `glob`, `check` and `Task` are observations, and an observation is what a model reaches for when it has nothing left to do and has been told to carry on. New alone let a prodded model look somewhere it had not looked; a change alone would let it re-run the same command.
...
**The test that encoded the old definition is corrected rather than deleted.** The bounded-carry-on test said a different call "is what makes it progress"; that premise is what this release disproves, and it now scripts a model that changes something each time. Two tests are new: a table of what counts as a change, and the measured failure itself.
...
**Retire returned while the turn was still running.** `Loop.Drain` waits on admission windows and background tasks, and a turn's admission window closes the moment the turn is registered — the handler answers 202 and leaves the turn to a goroutine. So Retire returned in under a millisecond with a reply still being written. The session stayed in `.handoff.json`, and nothing else ever writes that file, so the successor answered `409 still being finished by the previous localcode` for that conversation for the life of the window — forever, rather than "in a moment". And v0.103.0's `Store.Close` then closed the log under the running turn, so the model's answer went to a nil file handle and was dropped in silence: the conversation came back next start ending at the user's message. Retire waits for the turns now, keeps the manifest current while it waits, and closes the logs only if everything finished.

**The alive pipe was a local again, in three more places.** v0.101.0 fixed this in the window's handoff: held in a local the pipe is reachable only until the last statement that mentions it, and every caller passes it to `spawnSuccessor` and never touches it again — so the collector runs `os.File`'s finalizer, the successor reads EOF and calls `os.Exit(0)`. The two startup paths and the terminal's own daemon still made their own. All four use one per-process pipe, and a test fails on the next one to make its own.

**The second successor in a chain was killed by its own first diagnostic.** v0.101.0 changed the successor's stdio from inherited handles to a tee, so `os/exec` stopped passing a `*os.File` through and built a pipe instead, with the read end in the parent. A handoff makes a chain: generation 1 spawns generation 2 and exits, and generation 2's stderr belonged to a process that had gone. The next line it printed raised SIGPIPE, which Go makes fatal on descriptor 1 or 2.

**A daemon that handed off exited with status 1 microseconds later.** Closing the listener is what makes `Serve` return, and the goroutine watching it treated any return as fatal — so the process went down while Retire was still draining, killing every turn it had just promised to finish.

**The successor read the handoff manifest before the parent wrote it.** The read strictly preceded the write, every time, so the successor always started believing nothing was owned and never re-read a session the old daemon went on writing. Everything typed while the new version was starting disappeared from the conversation, and the successor's next append reused sequence numbers already in the file, which breaks `since=` replay and Last-Event-ID resume for that session permanently.

**A discarded daemon kept its scheduler and its session logs.** The window builds a whole daemon before it knows whether a newer binary is waiting, and "throws it away" only ever closed the MCP servers and the trace file. The scheduler stayed armed in a process that goes on running as the window, so every booked prompt fired twice; the logs stayed open, so on Windows deleting a conversation failed.

**Every second update in one Windows session failed.** The first renamed this process's own running image to `<exe>.old` and this process is still alive holding it. The aside name gets out of the way of the one before it now, and the copies are swept at the next start.

**One non-200 killed the event stream forever.** `EventSource` retries a transport drop by itself; it does not retry a reply that was not 200 `text/event-stream` — per the spec that fails the connection, and the browser never tries again. So one 404 for a session it did not know, or one 502 from a window whose successor had gone, left the page permanently deaf while looking alive: prompts posted, turns ran, and nothing was ever painted. The page reopens a stream the browser has given up on, with backoff.

**A turn whose daemon went away stopped being reported as running.** `session.waiting` is the page's own memory, cleared by the `turn.done` that answers it; if the daemon serving that turn goes away, that event never comes and the memory has no expiry. The reconnect is where the page stops trusting it and asks.

**The light no longer latches on a question that is gone.** `permission.resolved` closed the modal and unlocked the composer but left the pending id set, so after stopping a turn that was waiting on a tool call the dot went on saying "waiting for you to answer a permission request" with nothing to answer.

**Zoom defeated every cap in the stylesheet.** CSS `zoom` deliberately does not scale `vh` or `vw`, so `max-height:86vh` computed against the unzoomed viewport: at 1.5x the Settings panel was taller than the window with its Close button off the bottom and nothing to scroll. The container zoom this replaced shrank the viewport in CSS px instead, which is why the caps held before v0.100.0. Every viewport-relative size divides by the factor the page publishes, and a permission request — the one window with no way out — is capped and scrollable too.

**The window keeps its port.** Browser storage is partitioned by origin including the port, so a fresh ephemeral port every launch meant the zoom somebody set and the panel widths they dragged came back empty every time.

**Also:** a deleted conversation leaves every client's sidebar; a per-session reply that lands after you have moved on is dropped rather than applied; the page comes back on the new interface however the handoff was made; the install button comes back after an install that did not restart; cancelling the folder picker is not reported as a failure; the MCP servers stop before the successor connects when nothing is running; one `handoff.log` per generation, so a successor cannot truncate the file its parent is writing; a clean silent exit is no longer quoted as the cause of a 502; the splash says when the new version would not start; the successor is told that Windows will restart it; the debug log marks every change of hands so two concurrent calls stay separable, and a failure line starts on its own line; and one bash call is bounded rather than holding whatever the command chose to print.

**Not fixed, recorded as IMPROVEMENTS #46:** after a Windows startup handoff the installed copy under Program Files never updates again, and the settings panel says it is up to date. The reporting half is easy; making the install mean "replace the installed copy" needs the window process to run it, and by then the window has discarded its daemon. That is a decision rather than a patch.

## v0.103.0

**Three faults in the desktop window that only an update could reach, and a fourth that needed no update at all.** v0.101.0 fixed the pipe that had been killing the daemon behind the window, and in doing so turned on a mode that had never really run: before it, a successor died seconds after starting every time, so a window served its proxy for a few seconds at most. After it, windows run that way for hours, and three defects that had been sitting in that path came out together. This is why none of the four had been seen through v0.100.0 — not because the window's update worked then, but because it failed too early to reach them. The fourth, the latched indicator, came out of the same afternoon of running the app but is ordinary page code: any browser reaches it by stopping a turn that is waiting on a permission request, and its test drives no window and no proxy.
