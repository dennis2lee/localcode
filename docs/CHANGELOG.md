...
- **`localcode mcp import-claude`** pulls an existing Claude Code setup's MCP servers into localcode's config.json: this project's checked-in `./.mcp.json`, plus `~/.claude.json`'s global servers and its per-project block for the current directory (project-scoped entries win on a name collision, matching Claude Code's own precedence). Only stdio servers (`command`/`args`/`env`) can be represented; url-based remote servers are detected and skipped with a count, not silently dropped. Existing entries are left alone unless `--force`. `-s global|project` picks the target scope, same as `mcp add`.
