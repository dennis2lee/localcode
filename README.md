...
| Restart safety | Session list, conversation context, and `/usage` totals all restore from disk after a daemon restart. `/session` switches conversation from inside the TUI, which previously meant restarting it: the event stream is exchanged, and anything belonging to the conversation being left goes with it. |
