...
| Web UI | Left panel listing every session with the workspace it belongs to — click one to switch. Drag and drop file attach, model output rendered as markdown, Tab to switch agent, live status bar under the prompt (agent, model, context use, TPS, activity light, auto-delegate and permission toggles), workspace directory shown in the header (click for a native folder picker in the desktop window), and switching session moves the workspace to that session's project, right panel with background tasks and connected MCP servers |
| MCP | `localcode mcp add/list/get/remove` edits config.json for you, the same way `claude mcp` does. `mcp list` starts each server and reports whether it actually connects and what tools it advertises. `localcode mcp import-claude` pulls an existing Claude Code setup's MCP servers (`./.mcp.json` and `~/.claude.json`) straight in. |
