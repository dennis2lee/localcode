...
| Context limits | A request refused for being too long does not end the turn: the conversation is summarized, and if that is not enough it is trimmed, shrinking each time until the server accepts it. Sized against the conversation's measured length rather than a window figure, because the character estimate runs about 4x low on Korean and Japanese. Tool output is capped at a quarter of the window as it arrives, so one large file cannot fill the context by itself. Set `context_window` on a profile when the model's name gives no clue to its real limit. |
