...
| Desktop window | Experimental `--gui` opens the Web UI in a native OS window (no browser, no visible server). macOS ships a double-clickable `LocalCode.app` via `make dist-mac-gui`; Windows builds in CI. Built opt in with `-tags gui`. See [USAGE.md](docs/USAGE.md#desktop-window-experimental). |
