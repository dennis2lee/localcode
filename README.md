...
| Conversation | `/compact [instructions]` on demand, automatic compaction past 80% context, `/usage` for cumulative tokens per model (token counts only, no dollar figures). The three settings that change how a turn behaves each have a command of their own, answered by the daemon so both clients have them: `/smart-agent`, `/auto-delegate` and `/permission-skip-all` toggle with no argument, take `on` or `off`, and save the choice to config.json the way the settings window always has. |
