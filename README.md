...
| What is in the prompt | The system prompt is assembled from a declared inventory rather than concatenated: each piece has a stable id, a source, a trust class saying whether it may be followed as instruction at all, a place in the request, and a condition saying when it applies. `/context` reports the assembly for the next turn with no model call: what is included and why, per category token estimates, the tool definitions and conversation and reserved output against the window, and whether the request carries external content. `/context all` also says what was left out and why. Identities and sizes only, never the bodies. |
