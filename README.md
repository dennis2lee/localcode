...
| Desktop window | Experimental `--gui` opens the Web UI in a native OS window (no browser, no visible server). macOS ships a double-clickable `LocalCode.app` via `make dist-mac-gui`; the Windows MSI installs `localcode-gui.exe` plus a Start Menu shortcut and the WebView2 runtime bootstrapper. Built opt in with `-tags gui`. See [USAGE.md](docs/USAGE.md#desktop-window-experimental). |
