...
| Guards | With Smart Agent on: files that look like a credential store (`.env`, `*.pem`, `id_rsa`, `~/.ssh`, `~/.aws/credentials`, `.netrc` and the rest) are refused outright for read, write and edit, and `skip_permissions` does not unlock them, because it downgrades ask and never touches deny. A path outside the session's own workspace turns an allow into an ask rather than a refusal, judged on physical paths so a link inside the workspace aimed at `~/.aws` is outside, a file about to be created is judged by where its link leads, and a path that cannot be resolved at all costs one question rather than one blind allow. Both are overridden by writing an ordinary permission rule, and `skip_permissions` silences the boundary along with every other ask, because it is applied last. Every turn's system prompt also states which sources are instructions (you, the prompt, the project's rules) and which are data (every tool result), and MCP output arrives wrapped in a marker naming its server. That is labelling, not enforcement: the permission gate stays the control. |
