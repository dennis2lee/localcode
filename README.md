...
| Updating | A newer release is installed at startup and localcode comes back on it, before anything is served. `auto_update: false` turns it off. `/update` does it on demand without restarting what you are looking at: the new daemon takes the listening socket, the old one finishes its turns, the TUI and the browser reconnect where they were, and the next message goes to the new version. Two daemons never write one session at once. SHA-256 verified. Not on Windows, where a socket cannot cross processes and an MSI means a UAC prompt; there `/update` restarts and refuses while work is running. |
