...
| Updating | A newer release is installed at startup and localcode comes back on it, before anything is served (`auto_update: false` turns it off; not on Windows, where an MSI means a UAC prompt). `/update` does it on demand without restarting what you are looking at, on every platform: the new daemon takes the listening socket, the old one finishes its turns, the TUI and the browser reconnect where they were, and the next message goes to the new version. Two daemons never write one session at once. On Windows the running `.exe` is renamed aside, or staged under the user's cache directory where Program Files is not writable. SHA-256 verified. |
