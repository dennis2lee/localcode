...
| Running work | Each tool call gets a transcript line, written when it starts and completed with its result or a failure marker; click it for the full arguments and output. A line below the prompt also names the running tool and background tasks. `/tasks` inspects a background task mid run. |
...
| Dictation | Talk your prompt instead of typing it, entirely on your machine, with no network. `localcode dictation install` fetches a whisper.cpp engine and model; the engine runs as a child process, so this works in every build rather than only the desktop one. Grey text while you speak, committed when you pause. Korean and English, or both at once. See [USAGE.md](docs/USAGE.md#dictating-a-prompt). |
