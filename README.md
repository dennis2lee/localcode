...
| Project context | Session workspace rules from `AGENTS.md`, including `@path` imports, with `CLAUDE.md` as an equal second name. Skills, custom commands and global rules come from one home root: `~/.claude`, else `~/.opencode`, else `~/.localcode`. `/init` creates a draft. Automatic memory works across sessions. |
