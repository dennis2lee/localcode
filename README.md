...
| Updating | A newer release is installed at startup and localcode comes back on it, before anything is served and before any conversation is opened. That is the one moment it costs nothing: no turn is in flight and no background task is running. `auto_update: false` turns it off; `/update` does the same on demand and refuses while anything is running, naming what. SHA-256 verified, platform installer where there is one, binary replacement where localcode owns the file, `exec` back into the same terminal. Not at startup on Windows, where applying an MSI means a UAC prompt and no way back into the terminal. The settings window's manual check is unchanged and stays the only path over `--server`. |
