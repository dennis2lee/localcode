...
| Model quirks | A model whose id says it needs it gets one extra line of system prompt about this window. Gemma writes `$\rightarrow$` and `$\text{name}$`, which needs MathJax to read as anything; it is asked for the character instead, and the Web UI unwraps the ones that arrive anyway. No other model is told anything. |
